...
choose_format = Combobox(top_frame, values=['png', 'jpg', 'jpeg', 'webp'], width=6)
...
# Shortcut tip
shortcut_label = Label(shortcuts_frame, text='?', font=10, bg=BG_COLOR, fg='white')
...
shortcut = Checkbutton(shortcuts_frame, text='shortcuts', variable=shortcuts_on, command=shortcuts_toggle)
...
screen = Button(bottom_frame, text='Screenshot', height=2, width=10, command=take_screenshot)
screen.pack(side=LEFT, padx=6)

# dp = disappearance fix
dp_frame = Frame(master=bottom_frame, bg=BG_COLOR)
dp_frame.pack(side=RIGHT)

# If-does-not-work checkbox
idnw_on = BooleanVar()
idnw = Checkbutton(dp_frame, text='window hiding fix', variable=idnw_on)
idnw.pack(side=LEFT, padx=5)
